...
### Low priority
- **Decimales aceptados en campos `Integer` (Jackson)** — Al enviar un decimal (p. ej. `cantidad: 2.7`) en un campo `Integer` del request body, Jackson lo trunca hacia cero (`→ 2`) en vez de fallar, porque `DeserializationFeature.ACCEPT_FLOAT_AS_INT` está activo por defecto y la deserialización ocurre antes de `@Valid`. Desactivar con `spring.jackson.deserialization.accept-float-as-int=false` en `application.properties` y agregar en `GlobalExceptionHandler` un `@ExceptionHandler(HttpMessageNotReadableException.class)` que devuelva 400 (si no, caería en el handler genérico `Exception` → 500 engañoso).
...
- **Endpoint de creación de ventas** — `POST /api/ventas` (`VentaController.registrarVenta`) ahora existe; restringido a `ADMIN` y `VENDOR` en `SecurityConfig`. Recibe un `VentaDTO` (record con `productoId` `@NotNull` y `cantidad` `@NotNull @Positive`) validado con `@Valid`. `VentaService.registrarVenta` resuelve el `Producto` (404 vía `EntityNotFoundException` si no existe) y el `Usuario` desde el `Authentication` (`auth.getName()`), fija el `precioUnitario` desde el producto (`pr.getPrecio()`, no manipulable por el cliente), persiste y devuelve un `VentaResponseDTO` con `201 Created`. Los campos calculados por la BD (`total`, `fechaVenta`) se rellenan tras el insert porque la entidad `Venta` los marca con `@Generated(event = EventType.INSERT)`, que hace que Hibernate los re-lea automáticamente.
- **`System.out.println` removido de `ProductoService` y `VentaController`** — Eliminados los println de debug en `ProductoService.guardar` y en `VentaController` (filtro por fechas). Ya no queda ninguno en el código.
